...
serve(async (req) => {
  ...
  try {
    ...
    let productId = pack.stripe_product_id as string | null;
    if (!productId) {
      const created = await stripe.products.create({
        name: pack.nombre,
        description: pack.descripcion || `Paquete de ${pack.tokens} tokens de Kari (IA)`,
        metadata: {
          pack_id: pack.id,
    ...
    let priceId = pack.stripe_price_id as string | null;
    ...
    console.error("sync-ai-token-pack error", e);
